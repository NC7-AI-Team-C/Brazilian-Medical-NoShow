...
xgb = XGBClassifier(
    colsample_bylevel=0.3, colsample_bynode=0.9, colsample_bytree=0.9,
    gamma=0.05, learning_rate=0.01, max_depth=12,
    min_child_weight=9, n_estimators=500, reg_alpha=0.01,
    reg_lambda=0.5, subsample=0.9
) # randomsearchCV로 산출한 best_parameter 적용

lgbm = LGBMClassifier(
    feature_fraction=0.85, learning_rate=0.01, max_depth=3,
    min_data_in_leaf=1, n_estimators=700, num_leaves=24,
    reg_alpha=0.05, reg_lambda=1.2
) # randomsearchCV로 산출한 best_parameter 적용

cat = CatBoostClassifier(
    colsample_bylevel=0.9, l2_leaf_reg=9
    learning_rate=0.01, depth=9,
    n_estimators=500, border_count=10, bagging_temperature=1
    reg_lambda=0.1, subsample=0.5, random_strength=2
) # randomsearchCV로 산출한 best_parameter 적용
